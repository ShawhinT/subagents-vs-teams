...
- `task-N/agent-team/` — prompt and deliverables for the agent team run (multi-session); includes `.claude/settings.local.json` to enable agent teams
...
2. **Run agent team**: Open Claude Code inside `task-N/agent-team/`, paste `task-N/agent-team/prompt.txt`. The agent team prompt instructs Claude to delegate via agent teams rather than working alone. The `settings.local.json` in that directory enables the experimental agent teams feature.
